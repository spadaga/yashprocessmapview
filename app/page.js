...
export default function Page() {
  ...
  const renderDiagramViewer = (xml, isFiltered = false, filterColor = null) => {
    ...
    const viewerParams = new URLSearchParams({
      ...
      tooltips: "1",
      toolbar: "zoom layers",
      zoom: "0.8", // Reduced initial zoom for better fit
      pan: "0",
      tb: "0",
      ui: "kennedy",
      chrome: "0",
      ...
      setTimeout(() => {
        try {
          ...
          iframe.contentWindow.postMessage(
            JSON.stringify({ action: "zoom", zoom: 0.8 }),
            "*"
          );

          // Center with minimal padding
          setTimeout(() => {
            iframe.contentWindow.postMessage(
              JSON.stringify({ action: "pan", x: -20, y: -20 }), // Adjusted for minimal white space
              "*"
            );
          }, 150);
            ...
            <div className="flex items-center space-x-3 justify-center">
              ...
              >
                <svg
                  ...
                  strokeWidth={2}
                >
                  <rect x="4" y="4" width="16" height="16" rx="2" fill="#2563eb" stroke="#fff" />
                  <path d="M8 9l-2 3 2 3M16 9l2 3-2 3M11 12h2" stroke="#fff" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" />
                </svg>
                Change XML
              </button>
            </div>
            ...
            {showUpload && (
              <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-30 z-50">
                <div className="bg-white p-6 rounded shadow-lg flex flex-col items-center">
                  <label className="text-sm font-medium text-gray-700 mb-2">Upload XML:</label>
                  <input
                    type="file"
                    accept=".xml,.drawio"
                    onChange={e => {
